...
// Uso: node remux.mjs <entrada.mp4> <salida.mp4>
...
const outB64 = await pg.evaluate(async ({ base }) => {
  ...
  const ret = core.exec("-i", "in.mp4", "-f", "lavfi", "-i", "anullsrc=r=48000:cl=stereo", "-map", "0:v:0", "-map", "1:a:0", "-c:v", "copy", "-c:a", "aac", "-b:a", "64k", "-shortest", "-movflags", "+faststart", "-brand", "mp42", "out.mp4");
...
console.log(`Listo: ${outPath} (${(out.length / 1e6).toFixed(2)} MB, faststart)`);
