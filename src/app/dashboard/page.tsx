...
import { ExternalLink, FileText, MessageSquare, Package } from 'lucide-react'
...
const MOCK_PROJECT = {
  id: 'p1',
  name: 'Landing Page Panadería García',
  status: 'DEVELOPMENT',
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted text-sm mt-1">
            Proyecto #<span className="mono">{project.id}</span> · €{project.price}
          </p>
        </div>
        {project.demoUrl && (
          <a
      ...
      {project.demoUrl ? (
        ...
        <div className="border border-border p-4">
          ...
          <p className="text-muted text-sm">
            Disponible cuando el proyecto entre en fase de revisión.
          </p>
        </div>
      )}
        ...
        {[
          {
            ...
            icon: FileText,
            title: 'Briefing',
            description: 'Envía textos, logos y referencias',
          },
          {
            href: '/dashboard/tickets',
            icon: MessageSquare,
            title: 'Mensajes',
            description: 'Habla directamente con el equipo',
          },
          {
            href: '/dashboard/suscripcion',
            icon: Package,
            title: 'Suscripción',
            description: 'Gestiona tu plan de mantenimiento',
          },
        ].map((item) => {
      ...
      <div>
        ...
        <div className="relative">
          {[
            { status: 'LEAD', label: 'Solicitud recibida', done: true },
            { status: 'BRIEFING', label: 'Briefing completado', done: true },
            { status: 'DEVELOPMENT', label: 'En desarrollo', done: false, active: true },
            { status: 'REVIEW', label: 'En revisión', done: false },
            { status: 'DELIVERED', label: 'Entregado', done: false },
          ].map((step, i) => (
            <div key={step.status} className="flex items-start gap-4 pb-6 last:pb-0 relative">
              {i < 4 && (
                <div className={`absolute left-3 top-6 w-px h-full ${step.done ? 'bg-neon' : 'bg-border'}`} />
              )}
              <div
              ...
              <div className="pt-0.5">
                <p className={`text-sm font-medium ${step.done || step.active ? 'text-foreground' : 'text-muted'}`}>
                  {step.label}
                </p>
                {step.active && (
                  <Badge variant="warning" className="mt-1">En curso</Badge>
                )}
              </div>
            </div>
          ))}
