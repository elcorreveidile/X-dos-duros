import type { Metadata } from 'next'
import { Navbar } from '@/components/landing/Navbar'
import { Footer } from '@/components/landing/Footer'
import { Beautifier } from '@/components/tools/Beautifier'
import Link from 'next/link'

export const metadata: Metadata = {
  title: 'Beautifier de código gratis — Por 2 Duros',
  description:
    'Formatea y embellece código CSS, JavaScript y HTML. Herramienta gratuita para limpiar y organizar tu código.',
  alternates: {
    canonical: 'https://por2duros.com/herramientas/beautifier',
  },
}

export default function BeautifierPage() {
  return (
    <>
      <Navbar />
      <main className="min-h-screen bg-background text-foreground pt-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-16 space-y-12">

          <div className="space-y-3">
            <p className="text-xs uppercase tracking-widest font-mono text-neon">Herramienta gratuita</p>
            <h1 className="text-3xl sm:text-4xl font-black uppercase tracking-tight leading-none">
              Beautifier de<br />
              <span className="neon-text">código</span>
            </h1>
            <p className="text-muted text-sm max-w-lg leading-relaxed">
              Formatea y organiza código CSS, JavaScript y HTML comprimido. Añade
              indentación, saltos de línea y espaciado para hacer tu código legible.
            </p>
          </div>

          <Beautifier />

          <Link
            href="/herramientas"
            className="inline-flex items-center gap-2 text-xs text-muted hover:text-neon transition-colors uppercase tracking-wider"
          >
            ← Volver a herramientas
          </Link>

        </div>
      </main>
      <Footer />
    </>
  )
}
